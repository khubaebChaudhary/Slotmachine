...
def get_slot_machine_spins(rows, cols, symbols):
    all_symbols = []
    # This loop populates the list with symbols based on their count
    for symbol, symbol_count in symbols.items():
        for _ in range(symbol_count):
            all_symbols.append(symbol)

    columns = []
    # This loop creates each column of the slot machine
    for _ in range(cols):
        column = []
        current_symbols = all_symbols[:]  # Creates a copy of the symbols
        # This inner loop populates a single column
        for _ in range(rows):
...
def check_winnings(columns, lines, bet, values):
    winnings = 0
    winning_lines = []
    # Check each line that the user bet on
    for line in range(lines):
        symbol = columns[0][line]  # Get the symbol of the first column in the current row
        is_winning = True
        # Check if all other symbols in the row match
